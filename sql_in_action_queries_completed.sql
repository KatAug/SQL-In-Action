...
SELECT * FROM final_airbnb ORDER BY price ASC LIMIT 1;
...
SELECT * FROM final_airbnb WHERE NOT number_of_reviews;
...
SELECT id, room_type = "Private room" FROM final_airbnb ORDER BY number_of_reviews DESC LIMIT 1;
...
SELECT COUNT(id), neighbourhood FROM final_airbnb GROUP BY neighbourhood;
-- EXPECTED OUTPUT: Williamsburg

-- INVESTIGATE: Should Williamsburg be crowned the most popular neighbourhood?
-- Why is Williamsburg higher on the list than Harlem although they both have 16?? 
...
-- ALTER TABLE final_airbnb MODIFY COLUMN reviews_per_month int NOT NULL;
SELECT id, reviews_per_month FROM final_airbnb WHERE minimum_nights <7 ORDER BY reviews_per_month DESC;
-- EXPECTED OUTPUT: 58059
...
SELECT host_id, COUNT(host_id) AS host_name FROM final_airbnb GROUP BY host_id;
SELECT host_name, COUNT(host_id) AS How_many_listings FROM final_airbnb GROUP BY host_name;
...
-- <<<<<<<<<<<<<<<<<<<<< ** BONUS ** >>>>>>>>>>>>>>>>>>>>>>>
-- Find the percent above or below each listing as compared to the average price for all listings.
-- HINT: No hints! It's a bonus for a reason :)  
  
SELECT AVG(price) FROM final_airbnb WHERE minimum_nights;